...
describe('POST /api/categories', () => {
  it('should return 401 if headers are not legit', async () => {
    ...
    const req = new NextRequest('http://localhost/api/categories', { method: 'POST' });
    const res = await POST(req);
    expect(res.status).toBe(401);
  });

  it('should return categories if session is valid', async () => {
    (headersLegit as jest.Mock).mockReturnValue(true);
    (getAccountIDSession as jest.Mock).mockResolvedValue('acc123');
    const mockCategories = [{ category_id: 1, category_name: 'Food' }];
    (select as jest.Mock).mockResolvedValue(mockCategories);
