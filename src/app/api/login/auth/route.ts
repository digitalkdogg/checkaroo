    ...
    if (await validateUser(data.username, password)) {
        ...
        if (sessionstr) {
            if (await doesSessionExists(sessionstr, data.username)) {
                return NextResponse.json({'status': 'success', 'message': 'Session is valid already'})
            } else {
            ...
            if (await checkUserForActiveSession(data.username) == false) {
                ...
                if (login) {
                    return NextResponse.json({'status': 'success', 'message': 'Login Success! Redirecting now'})
                } else {return NextResponse.json({'status': 'error', 'message' : ' Unknown error'})}
            } else {
            //    return NextResponse.json({'testing': 'test'})
                expireSession(data.username);
                return NextResponse.json({'status': false, 'message': 'Hmmmm we can not login in at this time.  Try clearing your cache and try again.'})
            }
        }
    } else { //end validate user true
        return NextResponse.json({'status': false, 'message': 'Bad username and password.  Please try again'})
