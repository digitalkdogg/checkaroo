...
export async function POST(request: NextRequest) {
    ...
    const id = JSON.parse(decrypt(json.data)).clientid
    ...
    try {
        const transactions = await select({
            select: '*',
            from: 'Transactions',
            where: `account_id = "${accountid}" AND client_id = "${id}"`
        }) as Trans[]

        const clientHasTransactions = transactions.length > 0

        if (clientHasTransactions) {
            return NextResponse.json({
                message: 'Client is tied to one or more transactions',
                status: 'error'
            })
        }
